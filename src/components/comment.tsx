    ...
    return (
        <div className="project-view-comment" key={comment.comment_id}>
            <div className="project-view-comment-top-wrapper">
                <img style={{ width: '25px' }} src={comment.gh_avatar} alt={`${comment.username}'s profile`} />
                <Link to={`/user/${comment.username}`} className="project-view-comment-user">{comment.username}</Link>
            </div>
            {isCommentEditing ?
                <Form
                    form={form}
