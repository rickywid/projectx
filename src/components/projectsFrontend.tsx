...
const ProjectsFrontend = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      setIsLoading(true);
      const projectsFetch = await api.getProjectsByType("frontend", 1, 20);
  ...
  const fetchData = async () => {
    const res = await api.getProjectsByType("frontend", currentPage + 1, 20);
          ...
          {projects.length > 0 ? (
            <>
              <div className="tags-wrapper">
                <ResultsHeader count={resultsCount} name="Front End" />
                <ProjectsCard
                  projects={projects}
                  fetchData={fetchData}
                  hasMore={hasMore}
